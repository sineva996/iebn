...
parser.add_argument('--Temp', type=int, default=2, help='Temperature Coefficient of Knowledge Distillation.')
parser.add_argument('--alphaKD', type=float, default=0.8, help='loss factor.')
parser.add_argument('--db_head', type=int, default=1, help='Number of distributed workers.')
parser.add_argument("--exemplars", default=300, type=int, help="total number of exemplars")
parser.add_argument("--select", action="store_true",default=False, help="use herding selection")
...
def training():
    ...
    model = IEBN(num_workers=args.db_head)
    model.to(device)
    ...
    class_weight2 = torch.from_numpy( np.array([1,    0,   0,   1, 0.5,  0,  0.5]).astype(np.float32))   
    class_weight3 = torch.from_numpy( np.array([1.2,    0.5,   0.5,   1.2, 1,  0.5,  1]).astype(np.float32)) 
    ...
    """ params = list(model.parameters()) + list(af_loss.parameters())
    ...
    for itera in range(len(args.proc)):
        ...
        if itera > 0:
            ...
            exemplar_set = exemplar.Exemplar_rafdb(args.raf_path, args.order, itera, args.exemplars ,phase = 'train',transform = train_data_transforms)
        ...
        inc_train_dataset = dataset.Raf_increData(args.raf_path, args.order, itera,df ,phase = 'train', transform = train_data_transforms)
        inc_test_dataset = dataset.Raf_increData(args.raf_path,  args.order, itera,df ,phase = 'test', transform = test_data_transforms)
        ...
        for epoch in range(1, args.epochs + 1): 
            ...
            for (imgs, targets) in inc_train_loader:
                ...
                """ print("out:")
                print(out)#tensor[epoch,7]
                print("predicts:")
                print(predicts)#tensor[epoch]
                print("targets:")
                print(targets)#tensor[epoch] """
            ...
            if itera > 0:
                ...
                for (imgs,targets) in exemplar_loader:
                    ...
                    #loss = cro_loss(out,targets) + 1* af_loss(feat,targets) + 1*pt_loss(heads)
                    loss.backward()
                    optimizer.step() 
            ...
            if (acc_break > 0.95) and (epoch>8):
                break 


    Balance_Exemplar = Balance_Funtine.Balance_rafdb(args.raf_path, args.exemplars ,phase = 'train', transform = train_data_transforms)
    ...
    optimizer_balance = torch.optim.SGD(params,lr=0.5*args.lr, weight_decay = 1e-4, momentum=0.9)
    scheduler_blance = torch.optim.lr_scheduler.StepLR(optimizer_balance, step_size=8, gamma=0.1) 
    """ optimizer_balance = torch.optim.Adam(params,lr=0.5*args.lr, weight_decay = 0)
    scheduler_blance = torch.optim.lr_scheduler.StepLR(optimizer_balance, step_size=8, gamma=0.5) """

    print ("\n")
    print("Balance Training:")

    class_weight = torch.from_numpy( np.array([1.2,  1,  1.2,  1.2,  1.5,  0.9,  1.3]).astype(np.float32))    
